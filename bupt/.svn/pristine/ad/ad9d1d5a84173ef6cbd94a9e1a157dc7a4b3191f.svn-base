	...
	private void initView() {
		...
		btnSave.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				try {
					String saveImagePath = ((BaseApp) getApplication()).getSaveImagePath();
					ImageUtil.saveImageToSD(ImageZoomDialog.this,
							saveImagePath + ImageUtil.getTempFileName()
									+ ".jpg", bitmap, 100);
					ToastUtil.showMsg(ImageZoomDialog.this, "保存成功");
				} catch (IOException e) {
					e.printStackTrace();
					ToastUtil.showMsg(ImageZoomDialog.this, "保存失败");
				}
			}
		});
	}
		...
		final Handler handler = new Handler() {
			public void handleMessage(Message msg) {
				if (msg.what == 1 && msg.obj != null) {
					...
					ToastUtil.showMsg(ImageZoomDialog.this, ErrMsg);
					finish();
				}
			}
		};
			...
			public void run() {
				...
				try {
					...
					if (bmp == null) {
						...
						if (file.exists()) {
							bmp = ImageUtil.getBitmap(imgView.getContext(),filename);
						}
					}
					if (bmp == null) {
						bmp = HttpUtils.getNetBitmap(ImageZoomDialog.this,imgURL);
						if (bmp != null) {
							try {
								// 写图片缓存
								ImageUtil.saveImage(imgView.getContext(),filename, bmp);
							} catch (IOException e) {
								e.printStackTrace();
							}
							// 缩放图片
							bmp = ImageUtil.reDrawBitMap(ImageZoomDialog.this,bmp);
						}
					}
					msg.what = 1;
					msg.obj = bmp;
